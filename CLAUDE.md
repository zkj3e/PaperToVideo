...
```bash
# 仅渲染 PNG 截图
python scripts/render_slides.py <html文件> <页数> [--width=W] [--height=H] [--scale=N]

# 渲染 + 合成视频（HTML 含 data-time-* 属性时）
...
备选方案 — `compose_video.py`（不依赖 HTML 时间属性，从 SRT 自动分配时间）：
```bash
...
| `scripts/srt_to_qwentts_prompt.py` | SRT → 分段朗读稿（用于 TTS 配音） |
| `scripts/adjust_audio_speed.py` | 调整音频语速不变调（`--speed 1.1` 或 `--percent 110`） |
| `scripts/trim_video.py` | 视频裁剪到指定时长（`-d 26`） |
...
# 安装依赖
workspace/.venv/bin/pip install playwright && workspace/.venv/bin/playwright install chromium
```
